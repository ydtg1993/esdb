...
func SeriesDo() {
	...
	//索引的名称
	indexName := "movie_series"
	//redis的key
	redisKey := "synch_es_series"
	...
	//创建movie的index
	es.CheckIndex(indexName, es.MappingMovieSeries)

	//计算总记录数
	ma := new(models.MovieSeries)
	...
	lastid := 0
	for i := 0; i < max; i++ {
		var res []*models.MovieSeries
		mRes := map[string]*models.MovieSeries{}
		IDs := []string{}
		...
		if len(aids) > 0 {
			mCagegory := ma.GetMovieSeriesCategoryAssociate(fmt.Sprintf(" A.series_id in (%s) ", aids))
