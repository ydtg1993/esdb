...
func FilmDo() {
	...
	//索引的名称
	indexName := "movie_film_companies"
	//redis的key
	redisKey := "synch_es_film_companies"
	...
	//创建movie的index
	es.CheckIndex(indexName, es.MappingMovieFilm)

	//计算总记录数
	ma := new(models.MovieFilm)
	...
	lastid := 0
	for i := 0; i < max; i++ {
		var res []*models.MovieFilm
		mRes := map[string]*models.MovieFilm{}
		IDs := []string{}
		...
		if len(aids) > 0 {
			mCagegory := ma.GetMovieFilmCategoryAssociate(fmt.Sprintf(" A.film_companies_id in (%s) ", aids))
