...
func LabelDo() {
	...
	lastid := 0
	for i := 0; i < max; i++ {
		var res []*models.MovieLabel
		mRes := map[string]*models.MovieLabel{}
		IDs := []string{}
		...
		if len(aids) > 0 {
			mCagegory := ma.GetMovieLabelCategoryAssociate(fmt.Sprintf(" A.lid in (%s) ", aids))
