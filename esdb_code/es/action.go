...
func SearchLast(indexName string) string {
	ctx := context.Background()
	...
	if err != nil {
		logs.Error("读取时间节点错误 error->", indexName, err.Error())
		return ""
	}
	var obj map[string]interface{}
	var lastTime interface{}
	if searchResult.TotalHits() > 0 {
		for _, v := range searchResult.Each(reflect.TypeOf(obj)) {
			lastTime = v.(map[string]interface{})["created_at"]
		}
	} else {
		return ""
	}
